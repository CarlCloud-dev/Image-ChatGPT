...
const outDir = path.join(outRoot, "Image-ChatGPT");
const legacyOutDir = path.join(outRoot, "Image-ChatGPT-win32-x64");
const appDir = path.join(outDir, "resources", "app");
const preserveDir = path.join(outRoot, ".runtime-preserve");
const runtimeDirs = ["browser_profile", "browser_profile_edge", "output", "config"];
...
function ensureDependencies() {
  ...
  console.log("[deps] required runtime dependency missing; running npm install");
...
if (cleanRuntime) {
  console.log("[clean-runtime] not preserving browser_profile/browser_profile_edge/output/config");
} else {
  const previousOutDir = fs.existsSync(outDir) ? outDir : legacyOutDir;
  if (previousOutDir !== outDir && fs.existsSync(previousOutDir)) {
    console.log("[migrate] preserving runtime data from", previousOutDir);
  }
  for (const dir of runtimeDirs) {
    try {
      // Renaming on the same drive avoids temporarily duplicating a large
      // browser profile while the old portable folder is being rebuilt.
      moveIfExists(path.join(previousOutDir, dir), path.join(preserveDir, dir));
    } catch (e) {
      throw new Error(`Cannot preserve runtime directory "${dir}". Close Image-ChatGPT before building. ${e.message || e}`);
    }
  }
}
...
if (!cleanRuntime) {
  for (const dir of runtimeDirs) {
    moveIfExists(path.join(preserveDir, dir), path.join(outDir, dir));
  }
  ...
  if (!fs.existsSync(path.join(outDir, "browser_profile_edge"))) {
    copyIfExists(path.join(root, "browser_profile_edge"), path.join(outDir, "browser_profile_edge"));
  }
}
fs.rmSync(preserveDir, { recursive: true, force: true });
